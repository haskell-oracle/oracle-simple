...
import Database.Oracle.Simple.FromRow (FromRow, getRow, rDefineColumnTypes)
...
query_ :: forall a. FromRow a => Connection -> String -> IO [a]
query_ conn sql = do
  stmt <- prepareStmt conn sql
  dpiExecute stmt DPI_MODE_EXEC_DEFAULT
  rDefineColumnTypes @a stmt
